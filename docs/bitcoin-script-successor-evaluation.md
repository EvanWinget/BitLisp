...
7. **The review market prices novel consensus surface superlinearly, and it is on the record (added 2026-08-16).** In the 2025 CTV+CSFS thread (§8.5 item 3), TXHASH, a single opcode family, was judged an order of magnitude worse than CTV in technical debt and bug surface by the person who implemented both, independently seconded on review difficulty by a vault implementer, and the proposal's own 2025 rework then dropped TXHASH for a narrower opcode. Nobody argued expressiveness back against that pricing. LISP-C's matching engine is a larger increment of exactly the kind this market punishes hardest, and the §7.2 answer (specify once, vector adversarially, diff against a deployed validator) is a plan to pay the price, not evidence the price is low.
...
8. **The introspection side hand-builds this architecture per contract (added 2026-08-16).** The flexible-coin-earmarks design (§8.5 item 2), the introspection architecture's own state-carrying flagship, re-derives in per-contract script what the condition layer provides once: value accounting into a successor coin, output scriptPubKey recomputation byte for byte, and domain-separated state hashing, with a five-hook template layer proposed to make the plumbing bearable, a condition vocabulary in embryo. Our reading of the published example found its value check apparently broken (§8.5 item 2, our finding, flagged for re-verification). And the same author publicly defended BIP 345's deferred checks in 2023 on the ground that multiple inputs contributing safely to common outputs is something introspection templates can only mimic when every input follows a known template (§8.5 item 1), which is a transaction-level accounting object, the thing this side's validator makes first-class. The §6 item 5 exchange conceded the point a second time. When the shape demands accumulation, the introspection side reaches for a validator-level object every time it appears.
...
### 8.5 The public record, 2023 to 2025 (added 2026-08-16)

As with §8.4, everything here is public and cited as ordinary public record. Recorded from full readings of every message in the threads named below (2026-08-16). Each item states the fact and where it is consumed in this document or in the record docs.

1. **The introspection side endorses transaction-level accumulators when the shape demands them.** In the October 2023 thread on examining scriptPubKeys from Script ([archive](https://gnusha.org/pi/bitcoindev/ZTtgFPG4tTeZMnYn@erisian.com.au/T/#u)), Towns argued against Rusty Russell's template approach partly on the ground that BIP 345's deferred checks let multiple inputs contribute safely to common outputs, which introspection templates can only mimic when every input follows a known template. Deferred checks are a consensus-level accumulator across a transaction, the closest introspection-side artifact to this architecture's validator. Consumed by §7.2 item 8 and the §3.2 consolidation benchmark.
2. **Flexible coin earmarks is the hand-rolled-conditions exhibit.** [Delving 1275](https://delvingbitcoin.org/t/flexible-coin-earmarks/1275) (Nov 2024 to Jul 2025) carries state into a successor coin under introspection: the per-contract script must verify the earmark against a committed tree, compute remaining value, recompute the successor scriptPubKey from the new state root, and assert the output matches, with a five-hook bll template layer (Jul 2025) proposed to standardize the plumbing, and a stated requirement that the state tree hash in a way distinct from the BIP 341 TapLeaf/TapBranch tagged hashes "to ensure they can't be misinterpreted as taproot scripts, possibly enabling theft of funds." Our reading of the published example (`bllsh` `examples/test-flexmarks` at commit 04f924f against that commit's opcode table) found the value check apparently reading the output scriptPubKey field at the input's index rather than the output value at the supplied output index. That is our finding, not a thread statement, and it is re-verified against current upstream before any external citation. Consumed by §7.2 item 8 and by the D9 addendum in `docs/vm-record.md`.
3. **The 2025 review pricing.** In the CTV+CSFS thread ([post 14](https://delvingbitcoin.org/t/ctv-csfs-can-we-reach-consensus-on-a-first-step-towards-covenants/1509/14) and replies), Steven Roose, having implemented both, reported the difference in technical debt and bug potential between TXHASH and CTV as an order of magnitude, James O'Beirne independently called TXHASH's review an order of magnitude harder, and the proposal's July 2025 rework dropped TXHASH for OP_TEMPLATEHASH. Brandon Black in the same thread conceded that "CAT, Simplicity, or Lisp" would probably not harm the incentive structure while judging them politically unactivatable, so the general-language case stands unrebutted on merit and rejected on activation cost. Antoine Riard separately warned that selector-bitvector introspection modes are bit-level programming for application developers, an argument that favors any language with a real compiler. Consumed by §7.1 item 7 and the section 5 deployment posture, whose evidence-first two-track shape is the only generality argument that survived the thread.
4. **The bll architecture's own state, October 2024.** The bllsh announcement ([delving 1224](https://delvingbitcoin.org/t/debuggable-lisp-scripts/1224)) reversed bll to eager evaluation after the author's 2023 lazy preference and the March 2024 lazy prototype, with laziness surviving only as compiler sugar, and stated the consensus constraint that motivates the reversal's care: costing memory implies a consensus-preferred evaluation order, and the cost result "must not be changed as a result of any optimisations." The same post drops macro-based language construction as "something of a false idol" in favor of a fixed two-level language (bll and symbll) with a continuation-passing interpreter for native high-level debugging, responding to the Chia compiler maintainer's public advice to nail the high-to-low mapping early, Chia's self-described biggest tooling regret. Cohen's reply adds compiler requirements this project will meet at unit 7: `apply` bytes identical across language levels, program bytes fixed by running the compiler rather than by constraining it, and multi-target compilation for programs that commit to each other's hashes. As of that post the architecture had no published cost schedule, an unimplemented softfork operator, and a serializer pair marked broken. Consumed by the evaluation-order and D3 entries in `docs/vm-record.md` and by the Phase 3 compiler correctness story.
