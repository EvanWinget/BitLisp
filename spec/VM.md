...
operator intersection. Every deliberate difference carries a
divergence label (D1 to D9). The VM record in
[docs/vm-record.md](../docs/vm-record.md) holds the divergence table,
the oracle provenance, and the design decision record behind those
labels, and nothing in it is needed to predict a vector's outcome.
Every behavior in this document is pinned by vectors in
`vectors/vm/`.
...
- The message is the exact 32 bytes signed, never hashed again by
  ...
  recorded curation choice, the VM record's D2 entry: BIP348's
  CHECKSIGFROMSTACK instead accepts arbitrary-length unhashed
  messages).
...
- Verification follows BIP 340's Verify algorithm exactly, pinned by
  ...
  Core's test-framework implementation (provenance in the VM
  record).
