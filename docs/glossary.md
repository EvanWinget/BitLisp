...
| seal family (SEAL, SEAL_OUTPUTS) | nearest relatives are SIGHASH_ALL's whole-transaction commitment and BIP341's sha_outputs | no equivalent, Chia identity is content-derived so bundles need no seal | planned, ratified 2026-08-09, pins the spending transaction's own txid and the outputs hash respectively, lands as its own unit with the composition guarantee scoping it requires (condition-record decision 23, sequencing item) |
