...
- [ ] **Unit 0, the gate at phase open: fork-vs-scratch and implementation language.** Pre-registered evidence: the dialect debt in clvm_tools and clvm_tools_rs, Rue's maturity and stability at that date, and the v0 scope below. Lean recorded 2026-07-30: our own implementation at v0 scope, with Chialisp and Rue read freely as design references under the reference-material policy (copying code in stays prohibited, influence disclosed in commit messages). Python is the default implementation language: it keeps one toolchain, and it lets the Phase 5 playground run the real compiler and the real reference VM in the browser under Pyodide. The gate decision covers the whole phase, front end included, since the parser and printer it opens with are the compiler's front half.
- [ ] **Unit 1: parser and printer.** Text s-expressions to program trees and back, operator names per `VM.md`, round-trip property tests, and re-serialization pinned against programs from the existing vector corpus.
- [ ] **Unit 2: single-spend runner.** One command takes a program, a solution, and a transaction context, runs evaluation, condition parsing, and full validation, and reports the verdict, the emitted conditions, and the cost, or the error. Accepts text s-expressions from unit 1 or raw bytecode, and takes the transaction context in the validation corpus's shape.
- [ ] **Unit 3: REPL with stepper.** Interactive evaluation with the unit 2 transaction context loadable, a definitions scratch space, and a debugger: step, step-over, continue, trace. bllsh's command set is the reference point. From unit 4 onward this is the surface where language ergonomics are played with before they harden.
- [ ] **Unit 4: language core.** Named functions, constants, condition constants matching `CONDITIONS.md`, native `sha256tree` emission, compilation to bytecode, and compile-and-run tests against the reference VM. New constructs become usable in the REPL the same session they land.
- [ ] **Unit 5: macros, includes, currying helpers.**
- [ ] **Unit 6: the four benchmark puzzles, written in the language, as the acceptance artifact:** payment pool (benchmark 3), async offer (benchmark 6), vault, singleton wrapper. This phase makes them exist and run. Phase 4 measures them. One PR per puzzle where the size warrants it.
