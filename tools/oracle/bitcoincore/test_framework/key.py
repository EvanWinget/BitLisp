...
def sign_schnorr(key, msg, aux=None, flip_p=False, flip_r=False):
    ...
    kp = (
        int.from_bytes(TaggedHash("BIP0340/nonce", t + P.to_bytes_xonly() + msg), "big")
        % ORDER
    )
...
class TestFrameworkKey(unittest.TestCase):
    def test_ecdsa_and_schnorr(self):
        ...
        for msg in (
            byte_arrays
        ):  # test every combination of message, signing key, verification key
            for sign_privkey, _ in keys.items():
                ...
                for verify_privkey, verify_pubkey in keys.items():
                    ...
                    if verify_privkey == sign_privkey:
                        ...
                        self.assertTrue(
                            verify_schnorr(verify_xonly_pubkey, sig_schnorr, msg)
                        )
                        sig_ecdsa = random_bitflip(
                            sig_ecdsa
                        )  # damaging signature should break things
    ...
    def test_schnorr_testvectors(self):
        ...
        vectors_file = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "bip340_test_vectors.csv"
        )
            ...
            for row in reader:
                ...
                if seckey_hex != "":
                    ...
                    self.assertEqual(
                        pubkey.hex(),
                        pubkey_actual.hex(),
                        "BIP340 test vector %i (%s): pubkey mismatch" % (i, comment),
                    )
                    ...
                    try:
                        ...
                        self.assertEqual(
                            sig.hex(),
                            sig_actual.hex(),
                            "BIP340 test vector %i (%s): sig mismatch" % (i, comment),
                        )
                ...
                if result:
                    self.assertEqual(
                        result,
                        result_actual,
                        "BIP340 test vector %i (%s): verification failed"
                        % (i, comment),
                    )
                else:
                    self.assertEqual(
                        result,
                        result_actual,
                        "BIP340 test vector %i (%s): verification succeeded unexpectedly"
                        % (i, comment),
                    )
                num_tests += 1
