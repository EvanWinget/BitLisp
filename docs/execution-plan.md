...
- [ ] **Unit 5: currying and program identity.** Added 2026-08-15 (decision by Evan, skeleton change under ground rule 6): `curry` and `uncurry` as library and command surfaces, plus a tree-hash flag on the converters, the `opc -H` equivalent. Discrete from unit 4 (no call-semantics contact) and from macros (operates on programs, not source). The unit 7 puzzles consume all three. The Phase 4 commitment-hash deferral stands.
