...
- [x] **Unit 5: currying and program identity.** Added 2026-08-15 (decision by Evan, skeleton change under ground rule 6) and landed 2026-08-15 (PR 51): `curry` and `uncurry` as library, one-shot (`bitlisp-curry`, `bitlisp-uncurry`), and REPL surfaces, plus the `-H` tree-hash flag, the surfaces stated in `docs/lang/curry.md`. Decisions by Evan, 2026-08-15: the standalone commands ship beside the REPL commands, `-H` covers all three converters including `bitlisp-compile`, and the identity story gets its own doc. The curried shape and the uncurry sentinel contract match Chialisp tooling exactly. Discrete from unit 4 (no call-semantics contact) and from macros (operates on programs, not source). The unit 7 puzzles consume all three. The Phase 4 commitment-hash deferral stands.
- [ ] **Unit 6: macros and includes.**
