...
# The magnitudes adopt the deployed Chia condition-cost table where
# a deployed precedent exists.
CONDITION_GENERIC_COST = 200
CONDITION_MESSAGE_COST = 700
# PROVISIONAL: equals SECP_VERIFY_COST deliberately. Both constants
# price one BIP340 verification, so one Phase 4 measurement settles
# the price in both layers and a program can never buy the same
# verification cheaper as a condition than as an operator.
CONDITION_SIG_ASSERT_COST = SECP_VERIFY_COST
# PROVISIONAL: the point derivation the two taproot condition
# entries perform, a lift and one fixed-base multiplication. That
# is less work than a verification's two multiplications, so the
# verify magnitude is a deliberate overprice pending the Phase 4
# measurement.
TAPROOT_TWEAK_COST = 1_300_000
# PROVISIONAL: each output claim. Adopts the deployed Chia
# coin-creation magnitude, tuned there with production data against
# the same state-growth externality. Deliberately flat: a high
# linear price with the per-input budget as the cap.
